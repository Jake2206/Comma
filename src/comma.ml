(* 
  The Comma top-level which basically lets you run all the steps that have
  been built before.
  Shamelessly lifted from MicroC just to see if this bad boy will compile *)
   ...
   let () =
     ...
     let ast = Commaparse.program_rule Scanner.tokenize lexbuf in
     match !action with
